...
public final class SoftwareRenderer {
    ...
    public func render(_ commands: [FrameCommand]) {
        ...
        for cmd in commands {
            switch cmd {
            case let .rect(rect, color, _):
                ctx.setFillColor(cgColorFrom(color))
                ctx.fill(rect)

            case let .glyphRun(origin, text, fg, _, _):
                drawText(text, at: origin, foreground: fg, in: ctx)

            case let .cursor(rect, color):
                ctx.setFillColor(cgColorFrom(color))
                ctx.fill(rect)

            case let .selection(rect, color):
                ctx.setFillColor(cgColorFrom(color))
                ctx.fill(rect)
    ...
    private func drawText(_ text: String, at origin: CGPoint, foreground fg: UInt32, in ctx: CGContext) {
