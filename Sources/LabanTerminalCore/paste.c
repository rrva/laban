...
static void clear_paste_result(LabanPasteResult *out_result) {
    if (!out_result) return;
    out_result->bracketed = 0;
    out_result->bytes_written = 0;
}

static int laban_session_bracketed_paste_enabled_locked(LabanSession *s, int *out_enabled) {
    return laban_session_mode_active_locked(s, GHOSTTY_MODE_BRACKETED_PASTE, out_enabled);
}

int laban_session_bracketed_paste_enabled(LabanSession *s, int *out_enabled) {
    if (!s) return -1;
    SESSION_LOCK(s);
    return laban_session_bracketed_paste_enabled_locked(s, out_enabled);
}

static int laban_session_encode_paste_locked(
    LabanSession *s,
    const uint8_t *bytes,
    size_t len,
    uint8_t *out_bytes,
...
int laban_session_encode_paste(
    ...
    if (!out_bytes && out_capacity > 0) return -1;
    SESSION_LOCK(s);
    return laban_session_encode_paste_locked(
        s, bytes, len, out_bytes, out_capacity, out_len, out_bracketed);
}

static int laban_session_write_paste_encoded_locked(
    LabanSession *s,
    const uint8_t *bytes,
    size_t len,
    uint8_t *out_bytes,
    ...
    size_t enc_len = 0;
    int bracketed = 0;
    int r = laban_session_encode_paste_locked(s, bytes, len, buf, cap, &enc_len, &bracketed);
...
int laban_session_write_paste(
    ...
    if (!buf) return -1;
    size_t out_len = 0;
    int rc = laban_session_write_paste_encoded_locked(
        s, bytes, len, buf, cap, &out_len, out_result);
    free(buf);
    return rc == 1 ? -1 : rc;
}
...
int laban_session_write_paste_encoded(
    ...
    SESSION_LOCK(s);
    return laban_session_write_paste_encoded_locked(
        s, bytes, len, out_bytes, out_capacity, out_len, out_result);
}
