...
public enum TerminalHyperlink {
  ...
  static func plainHTTPURL(atRow row: Int, col: Int, in snapshot: LabanSnapshot) -> String? {
    ...
    let cols = Int(snapshot.cols)
    guard row >= 0, row < rows, col >= 0, col < cols,
      let rowText = visibleRowText(row: row, cols: cols, snapshot: snapshot),
      let clickedRange = rowText.cellRanges[col]
    else {
      return nil
    }

    let pattern = #"https?://[^\s<>"']+"#
    var searchRange = rowText.text.startIndex..<rowText.text.endIndex
    while let match = rowText.text.range(
      of: pattern,
      options: [.regularExpression, .caseInsensitive],
      range: searchRange
    ) {
      if let trimmed = trimPlainURLRange(match, in: rowText.text),
        rangesOverlap(clickedRange, trimmed)
      {
        let candidate = String(rowText.text[trimmed])
        if isBrowserHTTPURL(candidate) {
          return candidate
        }
      }
      searchRange = match.upperBound..<rowText.text.endIndex
    }
    return nil
  }
