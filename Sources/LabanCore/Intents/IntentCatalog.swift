...
public struct IntentCatalog: Sendable {
  ...
  private static let starterDescriptors: [IntentDescriptor] = [
    ...
    descriptor(
      ...
      summary: "Read native notification settings, counts, identity, and bounded diagnostics.",
      requiredCapability: .observe,
      dataSensitivity: .nonSensitiveState,
      availability: guiObserve,
    ...
    descriptor(
      ...
      risk: .init(
        level: .low,
        reason: "May display a banner, play a sound, or request notification authorization."),
      audit: .metadataOnly,
      availability: guiObserve,
      inputSchema: NativeNotificationTestRequest.jsonSchema,
      outputSchema: NativeNotificationTestAcceptedResponse.jsonSchema),
    descriptor(
      id: "tab.select",
      kind: .action,
      category: "tab",
  ...
  private static let legacyActionDescriptors: [IntentDescriptor] = [
    ...
    descriptor(
      id: "transparency.backgroundSource.set",
      category: "transparency",
      summary: "Set the requested terminal background source without replacing an image.",
      requiredCapability: .diagnosticControl,
      dataSensitivity: .nonSensitiveState,
      availability: guiObserve,
      inputSchema: SetBackgroundSourceActionRequest.jsonSchema),
    descriptor(
      id: "transparency.backgroundImageScaling.set",
      category: "transparency",
      summary: "Set managed terminal background image scaling without reimporting.",
      requiredCapability: .diagnosticControl,
      dataSensitivity: .nonSensitiveState,
      availability: guiObserve,
      inputSchema: SetBackgroundImageScalingActionRequest.jsonSchema),
    descriptor(
      id: "transparency.backgroundImage.import",
      category: "transparency",
      summary: "Import a contained fixture image into run-scoped managed storage.",
      requiredCapability: .diagnosticControl,
      dataSensitivity: .nonSensitiveState,
      availability: guiObserve,
      inputSchema: ImportBackgroundImageActionRequest.jsonSchema),
    descriptor(
      id: "transparency.backgroundImage.remove",
      category: "transparency",
      summary: "Remove the run-scoped managed image and select no native source.",
      requiredCapability: .diagnosticControl,
      dataSensitivity: .nonSensitiveState,
      availability: guiObserve,
    ...
    descriptor(
      id: "commandProposal.list",
      category: "command",
      summary: "List command proposals targeting the caller's own session.",
      requiredCapability: .propose,
      dataSensitivity: .visibleText,
      availability: guiObserve,
      inputSchema: CommandProposalRefRequest.jsonSchema,
      outputSchema: CommandProposalListResponse.jsonSchema),
    descriptor(
      id: "commandProposal.get",
      category: "command",
      summary: "Return one command proposal by id for the caller's own session.",
      requiredCapability: .propose,
      dataSensitivity: .visibleText,
      availability: guiObserve,
      inputSchema: CommandProposalRefRequest.jsonSchema,
      outputSchema: CommandProposalView.jsonSchema),
    descriptor(
      id: "commandProposal.cancel",
      category: "command",
      summary: "Cancel a pending command proposal; never writes PTY bytes.",
      requiredCapability: .propose,
      dataSensitivity: .visibleText,
      availability: guiObserve,
      inputSchema: CommandProposalRefRequest.jsonSchema,
      outputSchema: CommandProposalCancelResponse.jsonSchema),
    descriptor(
