...
`debugServer` is a Unix domain socket path, not a TCP URL. Agents should parse
this line instead of guessing socket paths, and must send the token as
`Authorization: Bearer <bearer-token>` on every debug request.
