...
The same setting also enables traveling-wave super-sampling: when the resolved color pattern over a single row translates at a steady velocity (a finely sampled source that already updates faster than the interpolation threshold), the Slug renderer re-renders the pattern at fractional cell offsets from a per-frame wave field instead of interpolating or snapping per cell, so the highlight glides continuously. Sources that do not exhibit a clean traveling wave are unchanged: sparse sources keep smoothstep transitions and finely sampled non-wave sources render authoritatively. When a wave disengages, cells ease from the wave-displayed color to the authoritative color through the same transition path.

A persisted setting enables a **sidebar hover preview** when the effective renderer is Slug Glyph. When enabled, hovering a background (non-active) tab's row in the sidebar shows a floating panel beside that row rendering a live, crisp miniature of that tab's most recent scrollback output, proportionally scaled from the terminal pane's current size and font. The panel disappears as soon as the pointer leaves the row, and never appears over the active tab's own row. The feature is disabled by default; other renderers, or the setting off, leave sidebar hover behavior unchanged. See `docs/adr/0031-sidebar-hover-preview-is-a-slug-capability.md` and `execplans/active/sidebar-hover-preview.md`.
...
Non-ASCII text matches literally without Unicode case folding. Soft-wrapped
rows are not joined in the first implementation, so a needle spanning a wrap
boundary is not a match.

Headless and capture/replay tooling must expose find state and find rendering.
The debug server provides `POST /debug/find/start`,
`POST /debug/find/step`, `POST /debug/find/stop`, and
`GET /debug/find/state`. Frame commands include `findMatch` and
`findSelected` rectangles so tests can assert the highlighted cells.

Deferred features include regex needles, Unicode case folding, pinned match
...
is not stored or exposed through debug state. Image scaling is persisted and
live-applied with exactly three choices: `Fill` (the default, proportional
...
image, scaling mode, or preset. The implementation and compositor evidence
requirements are tracked in
`execplans/active/terminal-background-transparency.md`, under “Active Work:
Background Sources and Frosted Preset.”
