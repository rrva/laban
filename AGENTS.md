...
This project is for building a minimal terminal application toward the behavior
described in `spec.md`, with the first cut constrained by `mvp.md`.

These instructions are deliberately practical. Prefer a working, inspectable
program over speculative architecture. Add to this file only when an agent
repeats a real mistake or when the project gains a concrete tool that agents
should use.

## Source of Truth

- `spec.md` is the long-term product behavior.
- `mvp.md` is the current scope boundary.
...
- If an agent repeatedly does the wrong thing, update this file or add a tool
  so the mistake becomes harder to repeat.
