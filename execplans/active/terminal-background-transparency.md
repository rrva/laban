...
The 2026-07-16 light-theme report reopened that narrow closeout. Commit
`cde484b` corrects the Vector/Slug replacement-canvas storage boundary. The
subsequent linear working-target follow-up closes the remaining glyph and
semantic source-over boundary while preserving those final bytes, and the
revised oracle at `2ca2e43` refuses single-theme evidence. Final closeout
requires a new exact installed artifact containing passing Selenized Light/Aqua
and Selenized Dark/Dark Aqua Slug results plus a fresh review of these changes;
the older dark-only artifact remains historical evidence, not proof of this
fix. The unrelated renderer-performance, CJK, and exact compositor-lane Review
Gate blockers remain unchanged.
...
- [ ] Install the light-theme fix and run the revised `rtk ./scripts/verify-system-blur-composition --app=$HOME/Laban.app --artifacts=.artifacts/transparency/system-blur-composition-<exact-commit>`; expect one schema-version-3 summary with exactly the required `selenized-light-aqua` and `selenized-dark-dark-aqua` results. Each result must attest exact theme name/darkness, Aqua/Dark Aqua appearance, configured/effective Slug, exact 0.30 None/System Blur state, retained source correlation, reduced amplitude-normalized edge sharpness, finite metrics, text-free crop/state hashes, exact provenance, JSON-null accessibility reset, process cleanup, and complete preference restoration. A dark-only run, a flat light capture, or the historical `d8560d0` artifact cannot satisfy this regression item.
