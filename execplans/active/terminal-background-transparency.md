...
- [x] (2026-07-15) Finish repository implementation closeout at `a8e0644`: the canonical full gate passes 444 parallel-safe tests plus 2,132 sequential tests with 16 expected skips and zero failures, the profilable release is installed at `/Users/user/Laban.app`, and all five Reduce Transparency and native-full-screen transition cycles pass.
...
- [x] (2026-07-15) Install profilable release `589b8ca` at `/Users/user/Laban.app` and pass five installed None↔System Blur↔Image source/opacity cycles, Fill/Fit/Stretch, missing-image repair, relaunch, Reduce Transparency, and native full-screen restoration at `.artifacts/transparency/transitions-589b8ca`.
- [x] (2026-07-15) Recalibrate Frosted to fixed opacity 0.30 at `e2cf5b3` after full-display measurements showed that the native material tint compounded 0.83/0.90 canvas tint into a nearly flat result; custom opacity remains literal. Fresh Sol review rejected the first oracle at `16b9829`, then found residual cleanup/provenance false positives after `04e9a85`. Commits `a7d5d48` and `d8560d0` close those gaps with in-memory crop-before-write, normalized edge sharpness plus a no-blur negative, sanitized launch state, signal-safe process-group cleanup, complete byte/type-exact preference restoration, HTTP/action success checks, exact JSON-null override reset proof, and exact build/OS/artifact/verifier/fixture/process-support provenance. The exact installed `d8560d0` run passes at `.artifacts/transparency/system-blur-composition-d8560d0`, and five installed transition cycles pass at `.artifacts/transparency/transitions-d8560d0` with the original opaque/None preferences restored.
- [x] (2026-07-15) Pass a fresh Sol follow-up review of the System Blur implementation and evidence at exact commit `d8560d065169ed674660a9a8db0237f39068a0fb`; no actionable findings remain. This narrow approval does not waive the overall Review Gate's unrelated renderer, CJK, or compositor-lane evidence requirements.
- [x] (2026-07-16) Fix the light-theme System Blur regression at `cde484b`: Vector and Slug now store translucent replace backgrounds and matching clears as encoded-sRGB premultiplied pixels instead of clipping bright canvases to white. All five renderers round-trip the Selenized Light Frosted canvas and remain stable over 100 full plus 100 partial frames; the composition analyzer reports a flat capture as finite failed JSON.
- [x] (2026-07-16) Strengthen the installed composition contract at `2ca2e43` to require two distinct pinned scenarios—Selenized Light/Aqua and Selenized Dark/Dark Aqua—with configured/effective Slug and exact theme/appearance identity in `/debug/transparency`. Both full-display in-memory crop measurements are required in one summary, and the complete defaults domain is still restored exactly.
- [x] (2026-07-16) Harden the paired installed composition oracle after the first cross-theme run exposed two harness defects: keep fixture Unix sockets below Darwin's path limit, start a newly fronted stripe backdrop for each scenario, order the exact controller window through the existing environment-gated GUI `windowFocus` action, retain bounded external activation/readiness polling, and refuse to encode or persist a crop unless the exact Laban window and expected backdrop are the first two intersecting windows in the complete front-to-back WindowServer stack.
- [x] (2026-07-16) Make the hardened oracle tolerate only the observed full-display Dock system surface: bind one leading layer-20 record to the live `com.apple.dock` process at `/System/Library/CoreServices/Dock.app`, require bounds exactly equal to the main display, match its exact PID/window ID/bundle/layer/frame in ScreenCaptureKit, and pass only that exact `SCWindow` to `excludingWindows`. Capture metadata schema 3 records the structural exclusion; every other layer and strict Laban/backdrop adjacency remain authoritative.
- [x] (2026-07-16) Correct the complete Vector/Slug translucent compositing boundary after fresh review: all nonopaque curve content now blends in a paired private `rgba16Float` linear-premultiplied working target and resolves once into the existing encoded-sRGB-premultiplied final target. Opaque activation compiles no new shader library or pipeline, allocates no working texture, and adds no pass. `TranslucentLinearBlendTests` proves the old head's bright grayscale edge, alpha, and semantic-overlay failures and the corrected path; the 17-test all-five parity/idempotence plus retained-damage set passes.
- [x] (2026-07-16) Close the follow-up color-glyph review finding: only the lazy translucent Vector/Slug color-glyph fragment now recovers straight encoded sRGB from the premultiplied-first Core Graphics atlas before linearization and one linear premultiplication. The shipped opaque shader and PSO remain unchanged. An actual-renderer emoji edge probe fails on `fb872d4` with a 49-byte RGB error for both renderers and passes after the correction; the expanded 18-test parity/idempotence/retained-damage set is green.
- [x] (2026-07-16) Install exact commit `f56ec857b5115f9a4a21cb1a786862c3cf7dbbd6` and pass the paired schema-version-3 composition oracle for Selenized Light/Aqua and Selenized Dark/Dark Aqua at `.artifacts/transparency/system-blur-composition-f56ec85/summary.json`. Installed build, repository HEAD, and recorded build commit all match; cleanup and complete preference restoration pass exactly. A fresh Sol follow-up review of that exact commit reports no findings.
- [x] (2026-07-17) Fix the production persisted-at-launch System Blur path at `f2280409`: coordinator init applies a persisted Frosted backdrop before the first layout pass, so the material view was created at zero bounds and behind-window blending never engaged; it is now reinstalled once at the first layout with real geometry. Focused host tests cover reinstall-once from zero bounds and no reinstall from real bounds; the user confirmed Frosted blur in the production Selenized Light theme on the installed build. The composition oracle's persisted-at-launch scenario gap is documented in Surprises & Discoveries and remains open.
- [ ] (2026-07-17) Replace the primary System Blur effect with runtime-resolved private CGS window blur, add a persisted Background blur slider, and redefine Frosted as 80% opacity plus 20% blur. The public material remains only the unavailable-symbol fallback. Focused tests, fresh paired composition evidence, and user visual comparison remain required before this item closes.
...
- Observation: Window-only ScreenCaptureKit filters suppress behind-window material composition and can make a working `NSVisualEffectView` look flat; AppKit hierarchy variants were equivalent under a complete-display capture. The remaining flatness at high canvas opacity came from the native material's own tint compounding the renderer tint, not from the host hierarchy. Raw edge-energy reduction alone is not proof of blur because a sharp affine low-contrast copy reduces that metric by the same amplitude factor.
  Evidence: Full-display stripe probes measured green-channel standard deviation `9.11` before the material, `6.24` at canvas opacity 0.30, `1.58` at 0.83, and `1.06` at 0.90. The rejected first oracle recorded the desktop and used raw edge energy, so its artifact is not acceptance evidence. The final fixture retains the complete-display filter only in memory, persists text-free terminal crops, rejects a sharp 5% affine tint in self-test, and measures direct/blur correlation `1.000/0.896`, amplitude `179.532/16.934`, and amplitude-normalized edge sharpness `1.000/0.0663` at `.artifacts/transparency/system-blur-composition-d8560d0/summary.json`.

- Observation: The initial installed oracle's dark-theme success did not cover the Vector/Slug sRGB storage boundary. With Selenized Light at Frosted alpha, those renderers stored `encode(linear(sRGB) * alpha)`; bright RGB bytes exceeded alpha, so WindowServer recovered clipped white and removed all visible System Blur contrast. Software, classic, and GPU-driven did not share the defect because their target storage is encoded-space premultiplied already.
  Evidence: The all-five bright-canvas parity and repeated-frame suites fail on the old Vector/Slug math and pass at `cde484b`; a pre-fix installed Slug crop was uniformly luma 255, while isolated software/classic/GPU-driven light runs retained correlated stripe signal.

- Observation: Correcting only the Vector/Slug replacement canvas storage did not correct later blending into the same sRGB target. Fixed-function blending decoded encoded-premultiplied destination bytes as `linear(sRGB * alpha)`, so bright antialiased glyph edges lost up to 34 RGB byte values, Vector preserved the canvas alpha with up to 158 alpha-byte error, and a semantic overlay missed its expected channels by 14–29 bytes.
  Evidence: `TranslucentLinearBlendTests` fails with nine assertions on the old head and passes when all nonopaque curve content composites in a linear-premultiplied float target followed by one final storage resolve. The all-five bright-canvas/idempotence suites and Slug retained-damage tests remain green.

- Observation: `ColorGlyphAtlas` uploads premultiplied-first Core Graphics bytes to a nonsRGB texture. Treating sampled RGB as straight sRGB and multiplying by sampled alpha again darkened partial-alpha emoji edges even inside the new linear working target.
  Evidence: The focused Vector/Slug color-glyph source-over probe compares actual final encoded-premultiplied bytes with the matching atlas texels and fails at `fb872d4` with a 49-byte maximum RGB error for each renderer. A translucent-only unpremultiply-before-linearize fragment makes both pass without changing `VectorGlyphShaders.metal` or any opaque PSO.

- Observation: A single persistent stripe window was not a safe two-scenario composition source. Closing the first Laban window made Codex frontmost; the second Laban then appeared above Codex while the old stripe window remained farther back, so the direct crop could include unrelated task text. The longer dark scenario control path independently exceeded Darwin's Unix-socket path capacity and surfaced as a modal `socketPathTooLong` startup failure.
  Evidence: Sampling the failed second launch reached `AppDelegate.showStartupFailure`, and Computer Use read the exact modal error. After shortening fixture IPC, the next paired run reached dark measurement but its direct crop visibly contained Codex while its System Blur crop was flat; it produced no summary and the invalid generated direct PNG was deleted immediately. The first strict-stack run then rejected a frontmost-window mismatch before writing any PNG, proving the persistence gate. A follow-up exact run showed `NSRunningApplication.activate` was ignored on this host and timed out with the same safe zero-PNG result. The hardened verifier now asks the exact live controller to `orderFrontRegardless()` through the existing GUI fixture credential, then leaves bounded external stack polling and final pre-encoding rejection authoritative.

- Observation: After exact controller focus succeeded, WindowServer still placed one persistent Dock-owned full-display layer-20 surface before Laban. Treating every nonzero layer as an occluder made the privacy gate impossible to satisfy even though the exact Laban and fresh stripe windows were adjacent beneath that system surface.
  Evidence: The front-to-back overlap records were exact Dock PID/window `28174/401`, layer 20, full display; exact Laban layer 0; then the exact fresh full-display stripe process layer 0. `NSRunningApplication` resolved the first process to bundle identifier `com.apple.dock` and the SIP-protected bundle path `/System/Library/CoreServices/Dock.app`; code signing identified the same Apple system bundle. The correction excludes only that exact leading structural match from the ScreenCaptureKit filter and rejects wrong PID, bundle, layer, undersized or oversized bounds, missing/duplicate shareable ID, or an otherwise eligible Dock record between Laban and the backdrop.
...
- Observation: the installed release exercises the real accessibility and native-full-screen transition machinery cleanly after repository closeout.
  Evidence: `rtk ./scripts/install-app` installed profilable release `a8e0644` at `/Users/user/Laban.app`; `rtk ./scripts/transparency-transition-smoke --app=$HOME/Laban.app --cycles=5 --artifacts=.artifacts/transparency/transitions-final` passed all five Reduce Transparency and all five native-full-screen cycles.

- Observation: the verified light/dark System Blur composition evidence did not cover the persisted-at-launch backdrop path. The composition oracle drives every transparency transition through post-launch fixture-authorized debug actions, so its material view is always installed after the window is on screen with real geometry. A production launch with Frosted persisted applies `.systemBlur` from coordinator init before the first layout pass; the `NSVisualEffectView` is then created at zero bounds, behind-window blending never engages on macOS 26, and the material renders only its flat Aqua/DarkAqua tint while requested policy, renderer canvas alpha, and accessibility state are all correct. Light themes made the flat result read as an opaque cream canvas, so the failure presented as "no System Blur with Selenized Light" while every oracle artifact was green.
  Evidence: installed-app self-captures showed the production canvas at `0.3 * bg0 + 0.7 * materialTint` in both themes (Selenized Light `(233,232,227)` vs opaque `bg0` `(251,243,219)`; Selenized Dark `(58,69,74)` vs `(16,60,72)`) with Reduce Transparency off, in-process snapshot capability, no full screen, and exact Frosted persisted settings. Commit `f2280409` reinstalls a zero-bounds material view once at the first layout with real geometry; focused host tests cover reinstall-once from zero bounds and no reinstall from real bounds, and the user confirmed Frosted blur in the production Selenized Light theme on the installed build. `scripts/verify-system-blur-composition` still has no persisted-at-launch scenario; adding one (pinned opacity/backdrop preferences before launch instead of post-launch debug actions) is the open follow-up.
...
Repository and installed-runtime closeout are green. At `589b8ca`, `rtk ./scripts/check` passes 448 parallel-safe tests plus 2,194 sequential tests with 16 expected skips and zero failures, 46.29% labpty MC/DC against the 45% floor, sanitizer, runtime smoke, and E2E. The four feature verifier self-tests and debug-contract check pass. The same implementation commit is installed as a profilable release at `/Users/user/Laban.app`; `.artifacts/transparency/transitions-589b8ca` passes five None↔System Blur↔Image cycles plus Fill/Fit/Stretch, missing-image repair, relaunch, Reduce Transparency, and native full-screen restoration. Sol's final expanded-scope implementation review reports no findings and approves `589b8ca`.

The System Blur follow-up is closed at `d8560d0`. Frosted now uses the fixed 0.30 canvas opacity that leaves the system material visibly useful, while custom opacity remains literal. The exact installed build passes the privacy-safe full-display composition oracle with direct/System-Blur correlation `1.000/0.896` and amplitude-normalized edge sharpness `1.000/0.0663`; its verifier, Swift fixture, imported process support, app executable, repository HEAD, OS, crop hashes, cleanup, and explicit JSON-null accessibility reset are mechanically bound in `.artifacts/transparency/system-blur-composition-d8560d0/summary.json`. Five final installed transition cycles also pass, and fresh Sol review approves exact commit `d8560d065169ed674660a9a8db0237f39068a0fb` with no actionable findings.

The 2026-07-16 light-theme follow-up is narrowly closed at exact commit
`f56ec857b5115f9a4a21cb1a786862c3cf7dbbd6`. Commit `cde484b` corrected the
Vector/Slug replacement-canvas storage boundary, the paired linear working
target corrected later glyph and semantic source-over, and the final
translucent-only color-glyph fragment corrected the premultiplied-first atlas
edge without changing the opaque path. The exact installed profilable build,
recorded build commit, and repository HEAD all match `f56ec857`; the paired
schema-version-3 artifact at
`.artifacts/transparency/system-blur-composition-f56ec85/summary.json` passes
Selenized Light/Aqua with normalized-edge ratio `0.10832181233415322` and
source correlation `0.8938702230427874`, and Selenized Dark/Dark Aqua with
ratio `0.06661453985263274` and correlation `0.8955740546547784`. It proves
owned process groups reaped, the preference domain restored byte-for-byte, and
the Reduce Transparency override restored to the system for every scenario.
Fresh Sol review approves that exact commit with no findings. The historical
dark-only artifact remains evidence of the earlier fix, not proof of this
paired regression. The unrelated renderer-performance, CJK, and exact
compositor-lane Review Gate blockers remain unchanged.
...
Apple's public contract is straightforward. An `NSWindow` that shows content behind it must not claim to be opaque, and a `CALayer` whose pixels can contain alpha must have `isOpaque == false`. Metal's existing `bgra8Unorm` and `bgra8Unorm_srgb` presentation formats carry alpha, so Laban does not need a new final pixel format. Vector and Slug do require a transient `rgba16Float` linear-premultiplied working target while nonopaque, followed by one resolve into encoded-sRGB-premultiplied final bytes. Keep source-over for glyphs/overlays and use overwrite clears and replace blending for background-establishing pixels so retained redraws are idempotent. Sources: [`NSWindow.backgroundColor`](https://developer.apple.com/documentation/appkit/nswindow/backgroundcolor), [`CALayer.isOpaque`](https://developer.apple.com/documentation/quartzcore/calayer/isopaque), and [`CAMetalLayer.pixelFormat`](https://developer.apple.com/documentation/quartzcore/cametallayer/pixelformat).
...
- Decision: The compositor profiler no longer records an in-process CPU
  profile (`cpu.perf`) per run. Its CPU and WindowServer numbers come from its
  own process sampling and the Metal trace, both unchanged.
  `verify-transparency-performance` requires only `metalTrace` and
  `metalAnalysis` trace paths, and still accepts a `cpuProfile` path in
  evidence recorded earlier.
  Rationale: The in-process sampler (`profile.capture`) was removed from
  Laban with its swift-profile-recorder dependency. Its profile was
  diagnostic evidence, not a gated metric.
  Date/Author: 2026-09-25 / Claude, at the user's request.
...
- Decision: Ship direct opacity plus one `NSVisualEffectView` system-blur mode behind the terminal canvas; do not use Liquid Glass for terminal content and do not implement configurable blur radius or private filters.
  ...
  Amended 2026-07-17: ADR 0029 reverses the no-private-filter/no-radius constraint for terminal window background blur only. The primary effect is runtime-resolved `CGSSetWindowBackgroundBlurRadius`; the public material remains the compatibility fallback, and Liquid Glass, screen capture, renderer shaders, and renderer-owned blur remain prohibited.
...
- Decision: Add a localized, theme-neutral `Frosted` preset with opacity 0.30, system blur, and explicit cell backgrounds kept opaque; do not auto-select it by locale and do not change the active theme. Amended after full-display compositor measurements showed the native material's tint compounded a 0.90 canvas tint into a nearly flat result (green-channel standard deviation `1.06` at 0.90 versus `6.24` at 0.30); arbitrary custom opacity remains literal.
  ...
  Amended 2026-07-17: Frosted is theme-aware instead of theme-neutral. Its canvas opacity is defined per theme brightness (0.30 dark, 0.55 light); applying the preset writes the active theme's value, and `FrostedPresetThemeFollower` re-resolves the persisted opacity on `Theme.didChangeNotification` while the requested controls still derive to Frosted. Custom slider values remain literal, and Frosted still never changes the active theme.
  Amended 2026-07-17: after direct visual comparison with Terminal.app, Frosted becomes 80% opacity plus 20% blur for both appearances. The tint-free private window blur removes the need for the earlier material-compensation opacities; custom opacity and blur values remain literal.
...
- Decision: Verify System Blur visually with a full-display ScreenCaptureKit filter over known rear stripes, never with Laban's app screenshot or a window-only capture filter.
  Rationale: Only complete-display capture preserves the WindowServer's behind-window material composition. The display remains memory-only and is cropped to the text-free terminal interior before persistence. Source correlation rejects an opaque/occluded terminal; amplitude-normalized edge sharpness distinguishes blur from a sharp low-contrast tint. The fixture uses the existing environment-gated diagnostic credential, a sanitized shell/environment, forced-off Reduce Transparency override, checked HTTP/action success, signal-safe owned process groups, exact complete-domain preference restoration, and build/OS/artifact provenance bound to the exact clean tracked verifier, fixture, and imported compositor process support at repository HEAD without broadening production routes.
  Date/Author: 2026-07-15 / Codex
  Amended 2026-07-16: the oracle must execute both pinned Selenized Light/Aqua and Selenized Dark/Dark Aqua scenarios with configured/effective Slug and exact debug-state identity. One scenario cannot substitute for the other.
  Amended 2026-07-16: each scenario owns a newly fronted stripe process. Immediately before each direct and blur capture, the verifier uses the existing isolated GUI fixture credential to send `windowFocus focused=true`; a MainActor handler orders that exact controller window front, while `false` is rejected. The external helper may also request public `NSRunningApplication` activation and polls the same strict validator for at most ten seconds; this is readiness, not a metric retry. Before capture and again before PNG encoding, every on-screen window layer intersecting the persisted crop is checked front-to-back: the exact Laban window must be first and the exact expected layer-zero stripe window must be second and cover the crop. Any occluder or intervening alert, panel, IME UI, popover, or application fails without persisting pixels.
  Amended 2026-07-16: permit at most one leading WindowServer surface only when the live owner resolves to exact bundle identifier `com.apple.dock` at `/System/Library/CoreServices/Dock.app`, its layer is exactly 20, and its bounds equal the complete zero-origin main display. The fixture must find that same window ID in `SCShareableContent`, revalidate exact PID/bundle/layer/frame, and give that exact `SCWindow` to `SCContentFilter(display:excludingWindows:)`; a missing or changed match fails before capture. Remove only this leading surface, then require exact Laban/backdrop adjacency. An eligible-looking surface between them is not removable. Capture metadata records IDs and structural identity but no window titles.

- Decision: Composite every nonopaque Vector/Slug frame in a paired private `rgba16Float` linear-premultiplied working target, then resolve once into the existing encoded-sRGB-premultiplied final target. Publish, present, and read back only the final target; retained damage repairs the corresponding working slot. Keep the opaque renderer on its original direct, lazily unchanged path.
  Rationale: Premultiplying after linearization lets bright stored RGB exceed alpha, but storing encoded-sRGB-premultiplied canvas bytes directly in an sRGB target makes later fixed-function blending decode the destination as `linear(sRGB * alpha)`. Only a full linear working-space composite followed by one storage conversion preserves both the Core Animation byte invariant and linear-light glyph/overlay behavior. Pairing ring slots preserves partial damage and one-frame-in-flight publication; lazy resources leave the default opaque cost unchanged.
...
“Requested configuration” below means values persisted from the user's settings. “Effective configuration” means the values actually applied after system policy. “Premultiplied alpha” means a target's red, green, and blue have been multiplied by alpha in that target's color representation; `FrameCommand` colors remain in the repository's existing straight-RGBA representation. The nonopaque Vector/Slug storage resolve converts linear-premultiplied working color into encoded-sRGB-premultiplied final color without reusing premultiplied RGB as straight input. “Explicit background” means a terminal application emitted a background color for a cell, including inverse video, rather than the cell merely inheriting the terminal's default canvas.
...
- **Background blur**, a 0–100% slider with a numeric percentage label. Default: 0%. Persist the unit interval as a `Double`, clamp invalid stored values to 0...1, map it directly to window blur radius 0...100, and coalesce it with pending opacity edits.
...
Add **Background source** (`None` / `System Blur` / `Image`) and **Preset** (`Opaque` / `Frosted`) to the Appearance tab. These sources are mutually exclusive. `None` preserves direct window transparency. `System Blur` primarily applies runtime-resolved private CGS window blur beneath Laban's translucent terminal pixels, with the prior public behind-window AppKit material retained only as the unavailable-symbol fallback. `Image` places one validated, user-imported local still image beneath that plane. The sidebar remains visually opaque. An opacity of 1.0 resolves to a fully opaque surface with zero active backdrop cost.
...
The background-opacity slider is the sole tint control for all three sources. The themed terminal canvas composites over Blur or Image, so lower opacity reveals more of the source; do not add an image-opacity slider. Background blur is independent of that tint and has no renderer effect. Choosing Image or changing image scaling produces custom preset state. Frosted remains exactly 80% opacity, 20% blur, System Blur, and opaque explicit cell backgrounds. Applying Frosted preserves the imported image and scaling for a later switch back; Frosted never combines Blur and Image. Custom opacity and blur values remain literal and are not compensated.
...
- The **Background source** popup (`None` / `System Blur` / `Image`), **Background blur** slider, and **Preset** control (`Opaque` / `Frosted`) in the Appearance tab. `System Blur` means runtime-resolved private CGS window blur with the prior standard macOS material as fallback, not Liquid Glass and not a renderer shader. `Image` means the managed local still image, not a terminal-frame image command. Backdrop source and opacity multiply rather than replace each other: the source supplies the backdrop and the themed Laban canvas tints it with the chosen opacity; None means direct window transparency.
- `Sources/LabanApp/TerminalWindowBlurController.swift`, owning runtime symbol resolution, radius clamping, and reset for private window blur. `Sources/LabanApp/TerminalBackgroundEffectHost.swift` owns zero or one child for Image or the public-material fallback. The fallback occupies only the terminal content rectangle and never extends under the opaque sidebar. AppKit/ImageIO/private-CGS types never leak into `LabanCore` or `LabanRenderer`. At effective opacity 1.0 or radius 0 there is no active backdrop cost.
...
- The `Frosted` preset: atomically selects 80% opacity, 20% blur, system blur, and leaves `Apply opacity to colored cell backgrounds` off. It never changes `Theme.current`, `Theme.followsSystemAppearance`, or the user's dark/light variants; it is localized in all 11 locales and never auto-selected from region, language, input source, or CJK font. Changing an individual control after applying a preset produces a custom state rather than mutating the preset definition.
- `laban-agent --background-blur=<0...1>` and `laban-agent --background-effect=<none|system-blur>` in the deterministic equals-form flag style.
...
- Constraints that bind the feature: no Liquid Glass behind terminal content, no screen capture, no renderer shader blur, and blur/compositing stays in AppKit/WindowServer rather than the terminal render loop. ADR 0029 permits only the runtime-resolved private CGS window-background radius path, with the prior public material as fallback.
...
Run all commands from `/Users/user/wrk/laban`. Follow the repository's machine-local command wrapper rule by prefixing shell commands with `rtk`.
...
- `./scripts/install-app` installs profilable release `589b8ca` at `/Users/user/Laban.app`, and `.artifacts/transparency/transitions-589b8ca` passes the five-cycle installed-app None/System Blur/Image, scaling, missing-image, relaunch, Reduce Transparency, and native-full-screen smoke.
...
- [x] Run `rtk swift test --filter TranslucentLinearBlendTests`; expect actual Vector and Slug renders to match linear-light source-over for bright grayscale glyph edges, premultiplied color-glyph atlas edges, and semantic overlays while retaining encoded-sRGB-premultiplied final bytes. The opaque activation case must prove that no translucent shader library/pipeline, working texture, or resolve pass is prepared until the surface becomes nonopaque.
...
- [x] Run `rtk ./scripts/verify-system-blur-composition --self-test`, then run `rtk ./scripts/verify-system-blur-composition --app=$HOME/Laban.app --artifacts=.artifacts/transparency/system-blur-composition-d8560d0`; both exit 0. The self-test rejects HTTP errors, action responses without `ok=true`, a false or absent override after reset, dirty verifier bytes relative to recorded HEAD, a sharp affine low-contrast copy, malformed/absent preference restoration, cleanup-error aggregation, deferred SIGHUP/SIGINT/SIGTERM, stubborn descendant escalation, and direct-child reaping. The installed oracle uses a complete-main-display `SCContentFilter(display:excludingWindows:)` but persists only in-memory text-free terminal crops; launches a sanitized deterministic shell/environment; forces effective Reduce Transparency false and proves the override itself is JSON null after reset; reports exact 0.30 None/System Blur state; preserves source correlation through blur; reduces amplitude-normalized stripe-edge sharpness; restores the complete preference domain byte-for-byte with exact plist types; binds app/build/repository/OS/executable/artifact identity plus exact clean verifier, fixture, and imported compositor process-support hashes and git blobs at recorded HEAD; and leaves no fixture process alive. Window-only, app-screenshot, persisted-desktop, unnormalized-edge, dirty-oracle, or otherwise unbound evidence cannot satisfy this item. The pre-final hardened runs remain diagnostic only.
- [x] Install the light-theme fix and run the revised `rtk ./scripts/verify-system-blur-composition --app=$HOME/Laban.app --artifacts=.artifacts/transparency/system-blur-composition-f56ec85`; `.artifacts/transparency/system-blur-composition-f56ec85/summary.json` is a passing schema-version-3 summary with exactly the required `selenized-light-aqua` and `selenized-dark-dark-aqua` results. Each result attests exact theme name/darkness, Aqua/Dark Aqua appearance, configured/effective Slug, exact 0.30 None/System Blur state, retained source correlation, reduced amplitude-normalized edge sharpness, finite metrics, text-free crop/state hashes, exact provenance, JSON-null accessibility reset, process cleanup, and complete preference restoration. Installed build, repository HEAD, and recorded build commit all resolve to exact `f56ec857b5115f9a4a21cb1a786862c3cf7dbbd6`.
...
- No runtime implementation defect was confirmed by the complete diff review, focused renderer/policy/headless/CJK/localization suites, five-backend parity matrix, installed-app transition smoke, or installed-bundle verification. `/Users/user/Laban.app` was installed from `111066d`, passed deep strict code-sign verification, and reported that commit in `LABANBuildCommit`.
...
- Item 12 is not passed. The exact command `rtk ./scripts/verify-transparency-cjk-evidence --manifest=.artifacts/transparency/cjk/cjk-evidence.json --spec=docs/product/spec.md` exits 1 with `verify-transparency-cjk-evidence: manifest is missing: /Users/user/wrk/laban/.artifacts/transparency/cjk/cjk-evidence.json`. Five renderer CJK PNGs exist, but the required manifest and Apple Pinyin evidence do not.
...
System Blur follow-up implementation review status: **APPROVED** — fresh Sol review completed on 2026-07-15 against exact implementation commit `d8560d065169ed674660a9a8db0237f39068a0fb`.

- Sol found no actionable findings after reviewing the production JSON-null reset projection, live-router regression, hardened oracle, imported process-support binding, and exact installed evidence. `TransparencyDiagnosticsTests` passes 4/4, the oracle self-test and Python compile pass, `git diff --check 5b0bf6c..d8560d0` passes, and `.artifacts/transparency/system-blur-composition-d8560d0/summary.json` proves current installed/repository identity, valid blur metrics, text-free persisted crops, and complete cleanup.
- This follow-up approval closes the System Blur visibility and evidence defect only. It does not reopen or supersede the third bounded overall Review Gate and does not waive its unrelated items 1, 12, or 14.

System Blur/light-theme follow-up review status: **APPROVED** — fresh Sol review completed on 2026-07-16 against exact implementation commit `f56ec857b5115f9a4a21cb1a786862c3cf7dbbd6`.

- Sol found no findings after reviewing the paired linear-premultiplied Vector/Slug working target, single encoded-sRGB-premultiplied resolve, and lazy opaque activation. The follow-up color-glyph correction recovers straight encoded sRGB from partial-alpha premultiplied-first atlas samples before linearization and one premultiplication; the actual-renderer regression fails at `fb872d4` with a maximum RGB error of 49 for both renderers and passes at `f56ec857`, while the shipped opaque shader and PSO remain unchanged.
- The full renderer suite passes 342 tests with 3 expected skips and zero failures; the combined transparency/parity/idempotence/Slug-retained-damage selection passes 18/18; `TranslucentLinearBlendTests` passes 4/4; and `./scripts/lint`, `./scripts/check-docs`, `./scripts/check-transparency-source-invariants`, and `git diff --check` are green.
- The exact installed schema-version-3 artifact at `.artifacts/transparency/system-blur-composition-f56ec85/summary.json` passes the required Selenized Light/Aqua and Selenized Dark/Dark Aqua scenarios with current installed/build/repository identity and exact cleanup/preference restoration.
- This approval is limited to the System Blur light-theme and curve-renderer follow-up. It does not rerun, reopen, or supersede the third bounded overall Review Gate, which remains **NOT PASSED**, and it does not waive the stable final renderer-performance artifact, source-specific CJK/Apple Pinyin manifest, exact base-M1/8-GiB/60-Hz compositor summary, exact macOS-27/Apple-silicon/120-Hz compositor summary, or observed macOS 27 lane-contract blockers.
...
- `system-blur-composition/summary.json`, direct/System-Blur text-free terminal-crop PNGs, and token-free state snapshots proving source correlation plus reduced amplitude-normalized stripe-edge sharpness through the real AppKit material; the complete display must never be persisted;
