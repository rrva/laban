...
- Decision: Verify System Blur visually with a full-display ScreenCaptureKit filter over known rear stripes, never with Laban's app screenshot or a window-only capture filter.
  Rationale: Only complete-display capture preserves the WindowServer's behind-window material composition. The display remains memory-only and is cropped to the text-free terminal interior before persistence. Source correlation rejects an opaque/occluded terminal; amplitude-normalized edge sharpness distinguishes blur from a sharp low-contrast tint. The fixture uses the existing environment-gated diagnostic credential, a sanitized shell/environment, forced-off Reduce Transparency override, signal-safe owned process groups, exact complete-domain preference restoration, and build/OS/hash provenance without broadening production routes.
  Date/Author: 2026-07-15 / Codex
...
- [x] Run `rtk ./scripts/verify-system-blur-composition --self-test`, then run `rtk ./scripts/verify-system-blur-composition --app=$HOME/Laban.app --artifacts=.artifacts/transparency/system-blur-composition-hardened-16b9829-run3`; expect both to exit 0. The self-test must reject a sharp affine low-contrast copy and exercise malformed/absent preference restoration, cleanup-error aggregation, deferred SIGHUP/SIGINT/SIGTERM, stubborn descendant escalation, and direct-child reaping. The installed oracle must use a complete-main-display `SCContentFilter(display:excludingWindows:)` but persist only in-memory text-free terminal crops; launch a sanitized deterministic shell/environment; force effective Reduce Transparency false and reset the override to null; report exact 0.30 None/System Blur state; preserve source correlation through blur; reduce amplitude-normalized stripe-edge sharpness; restore the complete preference domain byte-for-byte with exact plist types; bind app/build/repository/OS/fixture/executable/artifact identity; and leave no fixture process alive. Window-only, app-screenshot, persisted-desktop, unnormalized-edge, or unbound evidence cannot satisfy this item.
