...
- [ ] Implement the managed Image source in that same terminal-only AppKit host without adding a renderer image path, including import/relaunch/removal, Fill/Fit/Stretch, missing-file recovery, settings/debug/headless resolution, and zero-idle-cost coverage.
...
## Active Work: Background Sources and Frosted Preset

Direct background opacity is implemented. System Blur, managed Image, exact image scaling, and the `Frosted` preset are active incomplete deliverables in this ExecPlan. WindowServer material/image-host cost budgets and current/beta-OS compositor behavior remain on the critical path. The implementation must preserve every direct-opacity renderer, accessibility, full-screen, idle, compatibility, and evidence gate.
...
- Blur compositor scenarios and budgets: `Frosted` static WindowServer CPU median at most 8.0 percentage points above opaque static; `Frosted` animated at most 15.0 points above opaque animated. Image Fill/Fit/Stretch static runs each keep renderer-present delta zero, app CPU median below 1.0%, zero deadline misses, and WindowServer CPU median at most 2.0 percentage points above opaque static. Add installed-app Blur/Image screenshots, all three scaling modes, and compatibility evidence over each AppKit-backed source.
...
Implementation order: first finish the bounded AppKit Blur host and lifecycle tests; then add the pure source/scaling/availability contract and managed import store; then generalize the host to a mutually exclusive Image child; then expose settings/debug/headless resolution and the atomic Frosted preset with generated localizations; finally capture installed-window/scaling/compatibility evidence and both compositor lanes. No later slice may bypass host ownership, the managed-path boundary, or the opaque sidebar.
...
Extend that profiler and verifier with Frosted static/animated plus Image Fill/Fit/Stretch static scenarios over the same sampling windows and one checked-in high-resolution image fixture. Every native-source static run must remain parked with renderer-present delta 0, app CPU median below 1.0%, and no settled image decode/file-read counter growth. Median WindowServer CPU may be at most 8.0 percentage points above opaque static for Frosted static, at most 15.0 points above opaque animated for Frosted animated, and at most 2.0 points above opaque static for each Image scaling mode; the zero-deadline-miss requirements stay unchanged.
...
- The five-run compositor JSON passes these fixed gates: every 60-second static run has renderer-present delta 0 and app CPU median below 1.0%; every run has zero deadline misses; direct static and each Image Fill/Fit/Stretch static WindowServer CPU median are at most 2.0 percentage points above opaque static; Frosted static WindowServer CPU median is at most 8.0 percentage points above opaque static; and Frosted animated is at most 15.0 points above opaque animated.
...
- [ ] Run `rtk swift test --filter TerminalBackgroundEffectHostTests`, `rtk swift test --filter TerminalBackgroundImageStoreTests`, `rtk swift test --filter TerminalBackgroundImageScalingTests`, `rtk swift test --filter FrostedPresetTests`, `rtk swift test --filter TerminalBackgroundSourceSettingsUITests`, and `rtk swift test --filter BackgroundSourceLocalizationTests`; expect zero-or-one mutually exclusive terminal-only Blur/Image children, opaque-sidebar geometry, exact Fill/Fit/Stretch math/backing, atomic contained managed imports, safe missing/corrupt fallback, exact defaults under locale/input variations, and Frosted 0.90/System Blur/opaque explicit cells while preserving the managed image and changing individual controls to custom state. Run the installed transition smoke through None↔Blur↔Image and expect `backdropSubviewCount == 1` with the matching kind only while Blur/Image is effective and 0 otherwise.
...
- [ ] Run `rtk ./scripts/verify-transparency-performance --renderer=.artifacts/transparency/renderer-comparison.json --compositor=.artifacts/transparency/compositor/stable-base-m1-8gb-60hz/summary.json --lane-contract=fixtures/performance/transparency-stable-base-m1-8gb-60hz.json`; expect exit 0. The verifier is the sole gate authority for the 5% opaque, 10% translucent, and 8.33 ms renderer thresholds; every fixed CPU, present, deadline, direct/Blur/Image WindowServer threshold; zero settled Image decode/file-read growth; and exact equality of all required summary identity fields with the version-controlled lane contract. It also rejects a lane other than `stable-base-m1-8gb-60hz`, `osVersion` outside major 26, `osReleaseChannel` other than `stable`, `chipModel` other than base `Apple M1`, `memoryBytes` other than 8 GiB, or `displayHz` other than 60. A missing Fill/Fit/Stretch scenario, artifact, contract field, summary field, or verifier failure fails this item; do not substitute grep or manual arithmetic.
...
- `compositor/stable-base-m1-8gb-60hz/summary.json` and its trace paths for five 60-second runs of each opaque/direct/Frosted scenario and Image Fill/Fit/Stretch static scenario on the required hardware/OS lane, reporting app, WindowServer, renderer-present, deadline, and image decode/file-read counters separately;
