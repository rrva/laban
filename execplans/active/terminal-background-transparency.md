...
After this work, the Appearance settings offer a background-opacity slider, an opt-in control for applying opacity to explicitly colored terminal cells, a System Blur backdrop, and a localized, theme-neutral `Frosted` preset. A user can see a stable blurred backdrop through the terminal content while the entire sidebar remains a cohesive opaque navigation surface, can switch renderers without an opaque flash or semantic change, and can rely on Laban becoming fully opaque when macOS Reduce Transparency or native full-screen mode requires it. Headless screenshots and debug state make the renderer-neutral alpha behavior autonomously verifiable; installed-window evidence and compositor traces cover the native blur that cannot exist headlessly.
...
- [ ] Implement the AppKit-only System Blur effect host beneath the terminal content plane, including settings/debug/headless resolution, lifecycle, accessibility, full-screen, and zero-idle-cost coverage.
- [ ] Implement the theme-neutral localized `Frosted` preset and its custom-state transitions without changing theme, sidebar opacity, explicit-cell policy, or locale behavior.
- [ ] Capture blur-specific installed-window, CJK/IME, transition, and compositor evidence on every required lane; direct-opacity evidence does not substitute for the new material-backed scenarios.
...
As of 2026-07-15 the direct-opacity implementation and its repository regression closeout are complete, but the user-validated feature is not: System Blur and Frosted are now active incomplete scope. One requested/effective policy drives software, classic Metal, GPU-driven Metal, vector glyph, and Slug; all five use overwrite/replace background semantics, preserve opaque semantic regions, keep retained damage idempotent, flip presentation opacity live, and force vector-family grayscale antialiasing only while the surface is translucent. The direct Appearance controls, generated 11-locale catalog, AppKit accessibility/full-screen coordinator, fixture-only diagnostic authorization, headless parity, and installed-app transition flow are implemented and mechanically exercised. At implementation head `a8e0644`, the canonical direct-opacity full gate passes 444 parallel-safe tests plus 2,132 sequential tests with 16 expected skips and zero failures, including sanitizer, runtime-smoke, E2E, and 46.86% labpty MC/DC coverage against the 45% floor. The same commit is installed as a profilable release at `/Users/rrj/Laban.app`, where five Reduce Transparency and five native-full-screen cycles pass.

Commit `f05f1bd` incorporates the first live-validation amendment: the sidebar is fully opaque across terminal opacity settings, its memo no longer rebuilds for terminal-only compositing changes, and focused local/remote/warm/headless/CJK tests pass. The System Blur effect host, Frosted controls/preset, blur-specific evidence, and compositor lanes remain before completion.

Renderer correctness is no longer an outstanding direct-opacity implementation risk: focused suites, the five-backend authenticated parity matrix, atlas-budget coverage, GPU disjoint-damage coverage, capture/replay, and installed-app transition smoke pass. The vendored-cache defect behind DECXCPR and the two final stale gate assumptions are fixed. Overall acceptance nevertheless remains incomplete. The final-head balanced benchmark misses vector opaque wall p99 (`2.491208 ms` versus `2.128219 ms`), and the single post-diagnosis run moves the sole miss to vector direct wall p99 (`1.987666 ms` versus `1.858542 ms`) despite trace evidence that steady content GPU cost is below `0.45 ms` p99. Human validation supplied most Apple Pinyin captures, but the genuine 90% dark candidate case and final manifest remain, and the exact compositor command rejects this 16-GiB base-M1 host on `memoryBytes` before writing a summary. The third Review Gate remains the final bounded review of the earlier direct-opacity scope and stays **NOT PASSED**; new diagnosis and failed evidence do not convert code completeness, an older passing artifact, or a mismatched host into acceptance. The expanded blur/Frosted scope requires its own new gate items after implementation, not a reroll of those failed measurements.
...
- Decision: Ship direct opacity plus one `NSVisualEffectView` system-blur mode behind the terminal canvas; do not use Liquid Glass for terminal content and do not implement configurable blur radius or private filters.
  ...
  Date/Author: 2026-07-14 / Codex
  Amended 2026-07-14: the system-blur mode itself was deferred to the section now titled `Active Work: System Blur and Frosted Preset`. The no-Liquid-Glass, no-private-filter, and AppKit-host constraints were unchanged.
...
- Decision: Ship direct opacity only (the slider plus the explicit-cell opt-in); defer `System Blur`, the `Frosted` preset, the effect host, the `--background-effect` agent flag, and the 120 Hz macOS 27 seed compositor lane to a follow-up ExecPlan seeded by the section now titled `Active Work: System Blur and Frosted Preset`.
...
Add **Background effect** (`None` / `System Blur`) and **Preset** (`Opaque` / `Frosted`) to the Appearance tab. `None` preserves direct window transparency. `System Blur` places one public behind-window AppKit material beneath the terminal content plane; it does not extend under the opaque sidebar. An opacity of 1.0 resolves to a fully opaque surface with zero added effect cost.
...
## Active Work: System Blur and Frosted Preset

Direct background opacity is implemented, but live validation found it insufficient without backdrop blur. The system-blur backdrop and `Frosted` preset are therefore active incomplete deliverables in this ExecPlan. WindowServer material-cost budgets and current/beta-OS compositor behavior remain on the critical path. The implementation must preserve every direct-opacity renderer, accessibility, full-screen, idle, CJK, and evidence gate.
...
Already implemented as foundations: the `TerminalBackdropStyle.systemBlur` case, the resolver's `supportsBehindWindowBlur` and `headless` inputs with their resolve-to-`none` behavior, the requested/effective backdrop fields in `/debug/transparency`, the `backdropSubviewCount` diagnostic, and the grayscale-AA rule for any translucent or material-backed surface.

Implementation order: first add the bounded AppKit effect host and lifecycle tests; then expose settings/debug/agent control and headless resolution; then add the atomic Frosted preset and generated localizations; finally capture installed-window/CJK evidence and both compositor lanes. No later slice may bypass the effect-host ownership or opaque-sidebar boundary.
...
`TerminalBackdropStyle` keeps its `systemBlur` case and the resolver keeps its `supportsBehindWindowBlur` input. The visible app passes `supportsBehindWindowBlur: true` only after the effect host is installed and capable; headless remains false and resolves a stored System Blur request to effective `none`. `TerminalTransparencyPolicyTests` cover both supported and unavailable paths so AppKit capability cannot change resolver semantics.
...
Create `TerminalBackgroundEffectHost` in this plan with the ownership, terminal-only geometry, and one-child lifecycle recorded in `Active Work: System Blur and Frosted Preset`.
...
Expose the projection at `GET /debug/transparency`. Add four typed action names to the existing aggregate `POST /debug/actions` contract:
...
Add `Capability.diagnosticControl` and grant it only to `ControlTokenTier.fixture`; none of `appObserve`, `sessionObserve`, `approvedSession`, or `approvedSessionFamily` receives it. Explicitly reject/filter `diagnosticControl` from approval-request capability parsing so an approved-session capability array cannot acquire it. All four actions require that capability, use the environment-gated whole-app GUI fixture token, and are absent or forbidden for every other token. Preserve the existing validation rule that rejects `.fixture`-capability descriptors available in GUI; these descriptors use the narrower `diagnosticControl` capability instead. Add policy tests proving fixture-token success, non-fixture denial (including a forged approved-session capability list), GUI availability for all four actions, headless unavailability for `setNativeFullScreen`, and no permission broadening in `ControlLazyAttachAllowlist` or `ControlSessionObserveFamily`.

Counter deltas make one-observer, one-wake, and one-present requirements mechanical. `backdropSubviewCount` is instantaneous: exactly 1 only while effective System Blur is active, otherwise 0. Include the active snapshot-writer capability state (`inProcess`, `supported`, or `legacy`), real versus overridden Reduce Transparency state, native-full-screen state, and effective renderer identity in the projection. `GET /debug/accessibility` remains read-only.
...
First update `docs/product/spec.md` with the user-visible contract above. State that default opacity remains 1.0, every renderer must be equivalent, foreground content remains legible, accessibility and full screen can force opacity, and headless PNG alpha is part of the behavior. Record `System Blur` and the `Frosted` preset with the AppKit-only material and theme-neutral constraints. Do not edit the historical milestones/non-goals in `docs/product/mvp.md`.

Add `docs/adr/0028-terminal-background-transparency.md` and index it from `docs/adr/README.md`. The ADR records ownership across the window, AppKit effect host, frame request/producer, and renderer; the opaque-sidebar boundary; replace-versus-source-over command semantics; effective-alpha canvas overwrite for full frames versus transparent-black erasure for partial damage; the one-premultiplication/one-themed-tint invariant; explicit-background flag and mixed-version capability semantics; forced grayscale AA; the single accessibility-observer owner; system overrides; and why public native materials replace configurable private blur. Mark it accepted before later milestones and amend it whenever user validation changes these load-bearing boundaries.
...
rtk ./scripts/verify-transparency-performance --renderer=.artifacts/transparency/renderer-comparison.json --compositor=.artifacts/transparency/compositor/stable-base-m1-8gb-60hz/summary.json --lane-contract=fixtures/performance/transparency-stable-base-m1-8gb-60hz.json
```

Every command in this Milestone 4 block must exit 0. Run the compositor command on the machine matching its `--lane-contract`; the profiler detects the host identity, compares every field with the contract, and refuses a mismatch before sampling. Transfer only the resulting artifact directory back into `.artifacts/transparency/compositor/stable-base-m1-8gb-60hz/` if it was captured on another machine, then run the final verifier over the summary and version-controlled contract. If `./scripts/check` is not the current full gate when implementation begins, read `docs/process/agent-operating-guide.md`, replace it here with the canonical command, and record the change in `Decision Log`. `./scripts/build-app` proves only `.build/laban/Laban.app`; `./scripts/install-app` is required to update and validate `~/Laban.app`.

`transparency-transition-smoke` launches the installed app with the repository's environment-gated GUI control server, reads its whole-app fixture token, and uses the four `diagnosticControl` actions above. It registers shell-trap cleanup before the first mutation; cleanup removes the Reduce Transparency override, exits native full screen if necessary, and restores the initial requested settings. Before each measured operation it resets diagnostics, performs exactly one action, and polls `/debug/transparency` until the expected state settles. A requested-settings change must produce exact `effectiveTransparencyApplyCount`/`transparencyRenderWakeCount`/settled `rendererPresentCount` deltas of 1/1/1. A Reduce Transparency override must additionally produce `accessibilityRefreshCount == 1`. Native full-screen transitions may cause AppKit resize/animation presents, so their mechanical contract is instead: the effective force-opaque state changes once, the requested state is preserved/restored, `backdropSubviewCount` remains 0, and `rendererPresentCount` does not change during a two-second idle window after the transition settles. Run five real AppKit full-screen and five simulated Reduce Transparency cycles. The smoke also attempts each mutating action with app-observe and session-observe tokens and requires HTTP 403, proving the fixture-only authorization boundary on the installed binary.
...
- The five-run compositor JSON passes these fixed gates: every 60-second static run has renderer-present delta 0 and app CPU median below 1.0%; every run has zero deadline misses; direct static WindowServer CPU median is at most 2.0 percentage points above opaque; Frosted static WindowServer CPU median is at most 8.0 percentage points above opaque static; and Frosted animated is at most 15.0 points above opaque animated.
...
- [ ] Run `rtk swift test --filter TerminalBackgroundEffectHostTests`, `rtk swift test --filter FrostedPresetTests`, and `rtk swift test --filter TerminalTransparencySettingsUITests`; expect the effect host to own zero or one terminal-only material view, every forced-opaque/headless transition to remove it, Frosted to atomically request 0.90/System Blur/opaque explicit cells without changing theme or locale, and an individual edit to transition to custom state. Run the installed transition smoke extended with System Blur and expect `backdropSubviewCount == 1` only while the blur is effective and 0 otherwise.
...
- [ ] Run `rtk ./scripts/verify-transparency-cjk-evidence --manifest=.artifacts/transparency/cjk/cjk-evidence.json --spec=docs/product/spec.md`; expect exit 0. The verifier requires nonempty PNG evidence for all five exact renderer keys and nonempty Apple Pinyin artifacts, validates optional Rime/Squirrel artifacts when status is `passed`, and otherwise requires manifest status `notTested`, `compatibilityClaimed: false`, and the exact product-spec table status `not tested - compatibility unclaimed`. Performing the live flows and capturing evidence remains the executing agent's Milestone 3 responsibility, not this gate's.
- [x] Run `rtk ./scripts/transparency-transition-smoke --app=$HOME/Laban.app --cycles=5 --artifacts=.artifacts/transparency/review-transitions`; expect exit 0, constant-zero backdrop subview count, exact 1/1/1 effective-apply/wake/present deltas for requested-settings and Reduce Transparency transitions, one accessibility-refresh delta for each Reduce Transparency override, exact requested-setting restoration, and zero renderer-present growth during each two-second post-full-screen idle window.
- [ ] Run `rtk ./scripts/verify-transparency-performance --renderer=.artifacts/transparency/renderer-comparison.json --compositor=.artifacts/transparency/compositor/stable-base-m1-8gb-60hz/summary.json --lane-contract=fixtures/performance/transparency-stable-base-m1-8gb-60hz.json`; expect exit 0. The verifier is the sole gate authority for the 5% opaque, 10% translucent, and 8.33 ms renderer thresholds; every fixed CPU, present, deadline, and WindowServer compositor threshold; and exact equality of all required summary identity fields with the version-controlled lane contract. It also rejects a lane other than `stable-base-m1-8gb-60hz`, `osVersion` outside major 26, `osReleaseChannel` other than `stable`, `chipModel` other than base `Apple M1`, `memoryBytes` other than 8 GiB, or `displayHz` other than 60. A missing artifact, contract field, summary field, or verifier failure fails this item; do not substitute grep or manual arithmetic.
...
- `transitions/transparency-requested.json`, `transitions/transparency-reduce-transparency.json`, `transitions/transparency-fullscreen.json`, `transitions/transparency-restored.json`, and the remaining requested/effective/counter snapshots proving exact accessibility-refresh, apply, wake, and settled-present deltas plus a constant-zero effect-subview count;
...
- `windows/` containing labeled visible-window screenshots for opaque and 90% direct transparency in both a light and dark theme;
- `cjk/cjk-evidence.json`, the five exact `cjk/transparency-cjk-{software,classic,gpuDriven,vectorGlyph,slugGlyph}.png` files, labeled installed-window CJK screenshots over light and dark backdrops, `cjk/transparency-cjk-ime-apple-pinyin.*`, and—only when actually exercised—`cjk/transparency-cjk-ime-rime-squirrel.*`;
