...
- [x] (2026-07-15) Recalibrate Frosted to fixed opacity 0.30 at `e2cf5b3` after full-display measurements showed that the native material tint compounded 0.83/0.90 canvas tint into a nearly flat result; custom opacity remains literal. The first oracle at `16b9829` was rejected by fresh Sol review because it persisted the display, used an amplitude-sensitive metric, and had incomplete cleanup/preference/provenance contracts. Harden it with in-memory crop-before-write, normalized edge sharpness plus a no-blur negative, sanitized launch state, signal-safe process-group cleanup, complete byte/type-exact preference restoration, a forced-off accessibility override, HTTP/action success checks, exact null-override reset proof, and build/OS/artifact/verifier/fixture provenance. The hardened installed run at `.artifacts/transparency/system-blur-composition-hardened-16b9829-run3` predates the final verifier-binding amendment and must be regenerated from the final clean commit before being treated as final acceptance evidence.
...
- Observation: Window-only ScreenCaptureKit filters suppress behind-window material composition and can make a working `NSVisualEffectView` look flat; AppKit hierarchy variants were equivalent under a complete-display capture. The remaining flatness at high canvas opacity came from the native material's own tint compounding the renderer tint, not from the host hierarchy. Raw edge-energy reduction alone is not proof of blur because a sharp affine low-contrast copy reduces that metric by the same amplitude factor.
  Evidence: Full-display stripe probes measured green-channel standard deviation `9.11` before the material, `6.24` at canvas opacity 0.30, `1.58` at 0.83, and `1.06` at 0.90. The rejected first oracle recorded the desktop and used raw edge energy, so its artifact is not acceptance evidence. The hardened fixture retains the complete-display filter only in memory, persists text-free terminal crops, rejects a sharp 5% affine tint in self-test, and measures direct/blur correlation `1.000/0.896`, amplitude `179.532/16.934`, and amplitude-normalized edge sharpness `1.000/0.0663` at `.artifacts/transparency/system-blur-composition-hardened-16b9829-run3/summary.json`.
...
- [ ] Run `rtk ./scripts/verify-system-blur-composition --self-test`, then run `rtk ./scripts/verify-system-blur-composition --app=$HOME/Laban.app --artifacts=.artifacts/transparency/system-blur-composition-final`; expect both to exit 0. The self-test must reject HTTP errors, action responses without `ok=true`, a false or absent override after reset, dirty verifier bytes relative to recorded HEAD, a sharp affine low-contrast copy, malformed/absent preference restoration, cleanup-error aggregation, deferred SIGHUP/SIGINT/SIGTERM, stubborn descendant escalation, and direct-child reaping. The installed oracle must use a complete-main-display `SCContentFilter(display:excludingWindows:)` but persist only in-memory text-free terminal crops; launch a sanitized deterministic shell/environment; force effective Reduce Transparency false and prove the override itself is null after reset; report exact 0.30 None/System Blur state; preserve source correlation through blur; reduce amplitude-normalized stripe-edge sharpness; restore the complete preference domain byte-for-byte with exact plist types; bind app/build/repository/OS/executable/artifact identity plus exact clean verifier, fixture, and imported compositor process-support hashes and git blobs at recorded HEAD; and leave no fixture process alive. Window-only, app-screenshot, persisted-desktop, unnormalized-edge, dirty-oracle, or otherwise unbound evidence cannot satisfy this item. The pre-final hardened run is diagnostic evidence only because it predates exact verifier binding.
