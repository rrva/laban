...
`cde484b` corrects the Vector/Slug encoded-sRGB premultiplication boundary, and
the revised oracle at `2ca2e43` refuses single-theme evidence. Final closeout
...
- Decision: Store alpha-bearing replace backgrounds in Vector/Slug sRGB targets as encoded-sRGB premultiplied color, while preserving existing linear-light behavior for opaque colors, glyphs, and source-over content.
  Rationale: Premultiplying after linearization lets bright stored RGB exceed alpha; WindowServer then clips straight-color recovery toward white and can erase all visible backdrop contrast. Premultiplying in encoded space before target linearization satisfies the Core Animation storage invariant without changing gamma-correct opaque or glyph rendering.
