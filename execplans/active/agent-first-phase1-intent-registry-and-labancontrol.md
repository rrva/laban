...
- [ ] 1C-b/c remaining route-family port: internal `ControlRoute` handlers cover POST body routes, no-body mutation POST routes, `/debug/actions` families incl. `MouseActionResult`, and `laban-agent` mounts via `start(host:port:)` → `ControlReadiness`.
