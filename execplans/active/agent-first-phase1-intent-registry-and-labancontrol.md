...
- [ ] 1C-b/c remaining route-family port: internal `ControlRoute` handlers cover dynamic `/debug/sessions/<id>`, POST body routes, action families incl. `MouseActionResult`, binary artifacts via `ControlArtifact`, and `laban-agent` mounts via `start(host:port:)` → `ControlReadiness`.
