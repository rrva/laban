...
- Observation: The WindowServer natural-scroll-inverts the momentum-phase deltas of synthetic (CGEvent-injected) scroll streams but not the gesture-phase deltas. scrollbench `--flick` initially produced a sawtooth (gesture up, momentum down) with 181 spurious bottom-snaps; the injector now pre-inverts momentum deltas. Separately, the bench's SAD displacement matcher needed smallest-|shift| tie-breaking: near-identical adjacent ps-output rows produce equal-SAD minima at whole-row multiples, which read as phantom ±2-row jumps when true motion is ~2 px.
  Evidence: same trace, runs at t≈46.6–52.1 s (181 `snap`/`local-pin` events), vs. the clean real-flick run.
