...
Shipped: pixel-smooth precise scrolling (1:1 fractional tracking, whole-row settle at rest), a user-visible Scroll mode toggle in Settings (pixel-smooth default, line-quantized fallback), the `Tools/ScrollSmoothnessBench` on-glass harness with a phased `--flick` mode, and — the decisive find — a sign fix for `scrollContentYOffset` that had silently inverted all sub-cell scroll rendering since the smooth-scroll code was introduced. Verification stack that mattered, in order of ground-truth: on-glass capture (scrollbench) > app-internal per-frame state (ScrollDiagnostics `--scroll-debug`) > unit tests. The internal trace alone would have (and briefly did) exonerate a render-side bug; keep using on-glass measurement for anything perceptual. Remaining polish candidate, not scheduled: drive frames from the display link during active gestures so a 120 Hz panel gets 120 fps motion instead of event-rate (~60 fps).
