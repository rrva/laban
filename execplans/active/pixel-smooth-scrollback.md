...
- [x] On-glass scrollbench verification captured: post-fix flick tail is monotonic with smoothly decaying per-frame displacements and pixel-exact total distance (5244/5250 px); owner confirms "much better".

- Observation: With rendering exact, two residual stutter sources remained, both measured by scrollbench `--observe` (passive capture of manual scrolling): (a) velocity dropouts — frames with zero motion inside fast gestures followed by ~2× catch-up frames (16 in one 10 s session), and (b) a periodic ~10 Hz velocity ripple during slow scrolling (`-4 -4 -4 -2 -2 -2 -8` texture) from input-event timing beating against the frame clock. Raw 1:1 event tracking transfers input timing jitter directly onto glass velocity. Fix: the precise-input resampler — events only move the target accumulator; the rendered position chases it through the per-tick critically-damped controller with speed-adaptive stiffness (`adaptiveScrollOmega`, lag ≈ 2·v/ω kept under ~¼ row), so event gaps coast instead of freezing and catch-ups absorb instead of spiking.
  Evidence: `/tmp/scrollbench/laban-20260611-123612` and `-124026` (baseline); helper unit tests + reworked view tests.
- Observation: The explicit-Euler PD integration diverges once ω·dt exceeds ≈2 — the speed-adaptive stiffness (up to 400 rad/s) hit that limit instantly (`Int` conversion trap on the exploded position). Replaced with the closed-form critically-damped update, exact and unconditionally stable for any ω·dt.
- Observation (THE 60 Hz "REGRESSION" THAT WASN'T): an apparent halving of frame cadence (uniform 2-vsync deliveries, ~50% render duty, nextDrawable latency sharp at 15–16 ms) appeared the same hour the resampler landed and survived five renderer-side fixes. Root cause: **macOS Low Power Mode had engaged, hard-capping the ProMotion panel at 60 Hz** — drawable recycling locks to the panel's swap interval, so no app-side pacing can exceed it. The fixes shipped while chasing it are each independently sound (drop-don't-block scroll frames eliminate 8–24 ms main-thread stalls at capacity; prefetch-on-carry removes the render/miss alternation; the catch-up wake re-enters full rate after a missed-frame episode; paced presents declare cadence to the compositor) and with Low Power Mode off the chain measures 1121/1125 single-vsync deliveries (120 Hz), 3.3 ms/s hitch time, dropouts 16 → 5, nextDrawable latency 7.3 ms. Lesson, in order: check `pmset -g | grep lowpowermode` BEFORE debugging frame cadence; environmental coincidence fooled this investigation three times (renderer flip, scroll-mode flip, power mode).
  Evidence: journals 2026-06-11T13{03,11,25,33,35,46}*, 14{02,06}*; bench artifacts laban-20260611-{141913..160613}.

## Outcomes & Retrospective

Shipped, in order: pixel-smooth precise scrolling with a Settings toggle (pixel-smooth default); the `scrollContentYOffset` sign fix (inverted since the smooth-scroll code was born — the original "line stepping" complaint); the input resampler (events feed a fractional target; the rendered position chases it through a per-tick critically-damped follower with speed-adaptive stiffness and a closed-form integrator) which converts input-timing jitter into velocity-continuous motion; display-link-paced gesture rendering; and a present-pipeline pacing chain in `MetalDrawableScheduler`/`MetalRenderer` (drop-don't-block scroll frames, prefetch-on-carry, catch-up wake after misses, paced presents with `afterMinimumDuration`). Final on-glass measurement: 120 Hz deliveries (1121/1125 single-vsync), 3.3 ms/s hitch, fast-motion dropouts 16 → 5, total scroll distance pixel-exact vs commanded input.

Verification stack that mattered, in order of ground truth: on-glass capture (`Tools/ScrollSmoothnessBench`, `--flick` and `--observe`) > app-internal per-frame journal/trace > unit tests. The internal trace alone twice exonerated bugs the glass disproved. Three environmental coincidences derailed the investigation (renderer flip, scroll-mode flip, Low Power Mode); each was eventually caught by instrumentation, not reasoning. Check power state first when frame cadence is the symptom.
...
- Observation: A field report of "line-quantized stepping at the end of the gesture" in pixel-smooth mode was investigated with the `--scroll-debug` ScrollDiagnostics trace: the real trackpad flick showed a perfectly monotonic fractional target (445 events, zero direction flips, zero bottom-snaps), ~60 fps rendered frames with smoothly decaying per-frame deltas, continuous fraction sweeps across row boundaries, and a clean exponential settle — the pipeline is healthy. The reporter's `LabanScrollMode` was still `lineQuantized` from an earlier A/B (confirmed by their own redo note), which produces exactly that symptom: above one row/frame the modes are indistinguishable; the last ~3 rows of momentum decay are sub-row velocities where only fractional rendering shows motion.
  Evidence: `~/Library/Logs/Laban/scroll-trace/scroll-2026-06-11T08-08-24Z.jsonl`, runs at t≈59.5–66 s.
- Observation (CORRECTED): synthetic momentum-phase deltas are NOT natural-scroll-inverted by the WindowServer — an earlier inversion theory (and a short-lived bench "fix" pre-inverting momentum) was a misread of interleaved manual scrolling; the pre-inverted bench dragged the viewport to the bottom and pinned it (194 snaps, all-zero tail). Real bench findings: the SAD displacement matcher needs smallest-|shift| tie-breaking (near-identical adjacent ps-output rows alias as ±2-row phantoms at ~2 px true motion), and up-scroll motion reads as NEGATIVE displacement in the matcher's sign convention.
  Evidence: trace `scroll-2026-06-11T08-24-05Z.jsonl` run t≈19.1–22.3 s vs bench run laban-20260611-102428.
- Observation (THE ROOT CAUSE of "line stepping at gesture end"): `scrollContentYOffset` had an inverted sign since the smooth-scroll code was introduced. FrameProducer's cell coordinates are Y-up; the continuity requirement (same `displayed` rendered via adjacent applied/fraction splits must coincide on screen) fixes the sign to `+subCellRows*cellHeight`; the shipped `-subCellRows*cellHeight` made the glass creep half a row backward through each fraction sweep and leap two rows at every applied step. Invisible at ≥1 row/frame (fast scrolls, legacy PD glides), it surfaced as "switches to line-quantizing at the end of the gesture" once pixel-smooth mode made slow sub-row motion common. ScrollDiagnostics could not see it: it logs intended state (smooth), while the inversion lived between state and glass — only the on-glass bench caught it (`+2 +2 … −74` sawtooth, exactly 2 cell-heights). Locked by `FrameProducerScrollOffsetContinuityTests`.
  Evidence: bench laban-20260611-103915 (sawtooth) vs laban-20260611-105411 (post-fix: monotonic decaying tail, measured total distance 5244 px vs 5250 commanded).
- Observation (the fourth environmental trap): leaving BOTH debug surfaces armed — `LABAN_SCROLL_DEBUG` (set via `launchctl setenv`, so it silently re-arms on every relaunch) plus `LabanRenderJournalEnabled` — added 100–600 ms to the keystroke→echo round trip in every tab (ScrollDiagnostics logs each PTY feed from the reader thread under a lock the main thread also takes per frame; the trace file grew to 21 MB in one session). Scrolling stayed fast, which made it look like an unrelated input regression. Disarming both restored snappy typing. Lesson: `launchctl setenv` debug flags outlive the session that needed them; unset them the moment the investigation ends, and treat "input slow but rendering fast" as an instrumentation-overhead signature.
  Evidence: capture appkit-2026-06-11T14-34-27Z (keystroke→echo median 111 ms, p90 394 ms; echo→glass 55 ms), scroll-trace 2026-06-11T14-05-48Z.jsonl (21.2 MB).
...
- Observation (structural fix for the "needs you" typing lag): the attention pulse no longer touches terminal damage at all. The renderer gained a dedicated `sidebar-strip` render pass — one-shot `repaintSidebarStrip` flag (the `dropNextFrameWhenBusy` pattern), instances rebuilt from sidebar-source commands only, scissored to the strip width derived from the producer's full-strip background rect — encoded after either content pass, shared by classic and gpuDriven modes. Pulse-only frames carry `.partial([])` damage and repaint a ~200 pt strip instead of the full surface; the 30 fps full-damage rate-limit interim fix was removed along with attention's input into `shouldForceFullDamage`. Trace on the rate-limited build (e67b238, 20 s, needs-you tab pulsing + typing) already showed main-thread occupancy at 14.3% (from ~65%); the strip pass removes the remaining full repaints entirely. Locked by `MetalRendererSmokeTests.testSidebarStripRepaintPaintsStripWithoutTerminalDamage` (strip repaints on empty damage; terminal pixels byte-identical) and the rewritten `TerminalAttentionDamageTests`.
  Evidence: trace ~/Downloads/Untitled2.trace (2026-06-11 18:04); commit 56489dc.
