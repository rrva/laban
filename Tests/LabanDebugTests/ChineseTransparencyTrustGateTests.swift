...
final class ChineseTransparencyTrustGateTests: XCTestCase {
  ...
  private func assertPreeditSemantics(
    ...
    let preeditRuns = commands.compactMap { command -> (String, Int)? in
      guard case .glyphRun(_, let text, _, _, _, .preedit, _, _, _, let cells, _, _, _) = command
      else {
        return nil
      }
  ...
  private func cjkGeometry(in commands: [FrameCommand]) throws -> [CJKRunGeometry] {
    let terminalText = commands.compactMap { command -> String? in
      guard case .glyphRun(_, let text, _, _, _, .terminal, _, _, _, _, _, _, _) = command else {
        return nil
      }
      return text
    }.joined()
    ...
    let runs = commands.compactMap { command -> CJKRunGeometry? in
      guard
        case .glyphRun(
          let origin, let text, _, _, _, .terminal, _, _, _, let displayCellCount, _, _, _) =
          command,
