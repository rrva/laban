...
final class FrameProducerTests: XCTestCase {
  ...
  func testFixtureSessionWithHelloMvpProducesGlyphCommands() throws {
    ...
    let glyphCmds = cmds.compactMap { cmd -> String? in
      if case .glyphRun(_, let text, _, _, _, let src, _, _, _, _, _, _) = cmd, src == .terminal {
        return text
      }
      return nil
    }
  ...
  func testFixtureSessionProducesTerminalSourceCommands() throws {
    ...
    let terminalCmds = cmds.filter {
      ...
      if case .glyphRun(_, _, _, _, _, let src, _, _, _, _, _, _) = $0 { return src == .terminal }
      return false
    }
  ...
  func testNoBannerWhenRunning() {
    ...
    let exitLabels = cmds.compactMap { cmd -> String? in
      if case .glyphRun(_, let text, _, _, _, _, _, _, _, _, _, _) = cmd,
  ...
  func testBannerWhenExitedNormal() {
    ...
    let bannerLabels = cmds.compactMap { cmd -> String? in
      if case .glyphRun(_, let text, _, _, _, _, _, _, _, _, _, _) = cmd, text.contains("exited") {
        return text
      }
      return nil
    }
  ...
  func testSgrAttributesAppearOnGlyphRuns() throws {
    ...
    func attrs(matching needle: String) -> TextAttributes? {
      for cmd in cmds {
        if case .glyphRun(_, let text, _, _, let attributes, let src, _, _, _, _, _, _) = cmd,
  ...
  func testInverseVideoSwapsForegroundAndBackground() throws {
    ...
    for cmd in cmds {
      if case .glyphRun(_, let text, let fg, let bg, _, let src, _, _, _, _, _, _) = cmd,
        src == .terminal
      {
        if text.contains("FG") {
          normalFg = fg
  ...
  private func terminalGlyphRun(
    ...
    for cmd in cmds {
      if case .glyphRun(_, let text, let foreground, let background, _, let src, _, _, _, _, _, _) =
        cmd,
        src == .terminal, text.contains(needle)
      {
