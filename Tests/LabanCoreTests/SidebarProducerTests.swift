...
final class SidebarProducerTests: XCTestCase {
  ...
  func testAllCommandsHaveSidebarSource() {
    ...
    for cmd in cmds {
      ...
      case .glyphRun(_, _, _, _, _, let src, _, _, _, _, _, _, _):
  ...
  func testTabPositionsRenderAsDimGutter() {
    ...
    let runs = cmds.compactMap { cmd -> (CGPoint, String, UInt32)? in
      if case .glyphRun(let origin, let text, let fg, _, _, _, _, _, _, _, _, _, _) = cmd {
  ...
  func testTopTabGeometryStaysTopAnchoredAcrossResizeHeights() {
    ...
    for height in [620, 634, 760, 874] as [CGFloat] {
      ...
      let titleRun = cmds.compactMap { cmd -> CGPoint? in
        guard case .glyphRun(let origin, let text, _, _, _, let source, _, _, _, _, _, _, _) = cmd,
          source == .sidebar,
          text == "Tab 1"
        else { return nil }
        return origin
  ...
  func testExitedTabUsesdimForeground() {
    ...
    let labelRuns = cmds.compactMap { cmd -> (String, UInt32)? in
      if case .glyphRun(_, let text, let fg, _, _, _, _, _, _, _, _, _, _) = cmd, text == "zsh" {
        return (text, fg)
      }
      return nil
    }
  ...
  func testRunningActiveTabUsesNormalForeground() {
    ...
    let cmds = p.commands(tabs: tabs, activeTabId: tabs[0].id, height: 600)
    let labelRuns = cmds.compactMap { cmd -> (String, UInt32)? in
      if case .glyphRun(_, let text, let fg, _, _, _, _, _, _, _, _, _, _) = cmd,
        text.contains("Tab")
      {
        return (text, fg)
      }
  ...
  func testNotificationStackDoesNotSpillIntoNextTab() {
    ...
    let origins = cmds.compactMap { cmd -> CGPoint? in
      if case .glyphRun(let origin, _, _, _, _, _, _, _, _, _, _, _, _) = cmd { return origin }
      return nil
    }
  ...
  // `SidebarProducer.output(...)` no longer takes a `hoverPreview` parameter
  // — production resolves the panel's chrome via the standalone
  // `hoverPreviewCommands(...)` static function directly
  // (`TerminalSurfaceController.hoverPreviewOverlayCommands` calls it, not
  // `output`), so these tests call the same function production actually
  // uses instead of a parallel, otherwise-unreachable code path.
  ...
  func testNilHoverPreviewEmitsNoPreviewCommands() {
    ...
    let commands = SidebarProducer.hoverPreviewCommands(
      tabs: tabs, activeTabId: tabs[0].id, height: 600, topInset: 0, scrollOffset: 0,
      rowHeight: p.rowHeight, sidebarWidth: p.sidebarWidth, hoverPreview: nil)
    XCTAssertTrue(commands.isEmpty, "a nil hoverPreview must never emit .sidebarPreview commands")
  }
