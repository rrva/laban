...
final class LabanRendererSmokeTests: XCTestCase {
    ...
    func testAllFrameCommandTypesCanBeConstructed() {
        ...
        let cmds: [FrameCommand] = [
            .rect(rect, color: 0xFF0000FF, source: .terminal),
            .glyphRun(origin: .zero, text: "A", foreground: 0x000000FF, background: 0xFFFFFFFF, source: .terminal),
            .cursor(rect, color: 0x3A4D53FF),
            .selection(rect, color: 0xECE3CC80),
            .clip(rect),
    ...
    func testTexturedQuadCommandCarriesResourceId() {
        ...
        if case let .texturedQuad(_, resourceId, source) = quad {
    ...
    func testRectCommandFillsExpectedRegion() {
        // 16x16 surface; fill only the top-right quadrant (CG coords: x=[8,16), y=[8,16))
        let surface = BitmapSurface(width: 16, height: 16)
        let renderer = SoftwareRenderer(surface: surface, fontAtlas: FontAtlas())
        renderer.render([
            .rect(CGRect(x: 8, y: 8, width: 8, height: 8), color: 0xFF0000FF, source: .terminal),
        ])

        // Pixel inside the filled quadrant must be red
        ...
        // Pixel in the opposite (unfilled) quadrant must remain zero
...
// MARK: - Glyph rendering
    ...
    func testGlyphCommandProducesNonBackgroundPixels() {
        ...
        // Fill background, then draw a glyph on top
        let bg: UInt32 = Theme.SelenizedLight.bg0
        renderer.render([
            .rect(CGRect(x: 0, y: 0, width: CGFloat(cellW), height: CGFloat(cellH)), color: bg, source: .terminal),
            .glyphRun(origin: .zero, text: "A", foreground: Theme.SelenizedLight.fg1, background: bg, source: .terminal),
        ])

        // At least one pixel somewhere in the cell must differ from the background
    ...
    func testPNGBytesHaveValidSignature() {
        ...
        let renderer = SoftwareRenderer(surface: surface, fontAtlas: FontAtlas())
        renderer.render([.rect(CGRect(x: 0, y: 0, width: 4, height: 4), color: 0x123456FF, source: .terminal)])
    ...
    func testPNGFromClearSurfaceIsNonEmpty() {
        ...
        SoftwareRenderer(surface: surface, fontAtlas: FontAtlas()).render([
            .rect(CGRect(x: 0, y: 0, width: 80, height: 24), color: Theme.SelenizedLight.bg0, source: .terminal),
        ])
        guard let data = surface.pngData else {
    ...
    func testCursorCommandFillsCell() {
        ...
        let renderer = SoftwareRenderer(surface: surface, fontAtlas: FontAtlas())
        renderer.render([
            .rect(CGRect(x: 0, y: 0, width: 10, height: 18), color: Theme.SelenizedLight.bg0, source: .terminal),
    ...
    func testLargeGridSmokeDiagnostic() {
        ...
        // Build a command list simulating a full terminal screen of text
        var cmds: [FrameCommand] = []
        cmds.append(.rect(CGRect(x: 0, y: 0, width: CGFloat(width), height: CGFloat(height)),
        ...
        print("[smoke] large-grid: \(cmds.count) commands, \(String(format: "%.1f", elapsed * 1000))ms, \(cols)x\(rows) cells")
